...
def diffusion_curve_axis(ax, ax_label_size, w_10, profile_dict, diffusion_type, color, linestyle='dotted',
                         gradient=False, wave_roughness=False, theta=1.0):
    """
    ...
    profile = utils.get_vertical_diffusion_profile(w_10, depth * -1, diffusion_type, wave_roughness=wave_roughness,
                                                   theta=theta)
    ...
    if gradient:
        ...
        profile = utils.get_vertical_diffusion_gradient_profile(w_10, depth * -1, diffusion_type, theta=theta,
...
def base_figure(fig_size, ax_range, y_label, x_label, ax_label_size, shape=(1, 1), plot_num=1, all_x_labels=False,
                legend_axis=False, log_xscale=True, width_ratios=None):
    """
    ...
    for w_rise in w_rise_list:
        for w_10 in w_10_list:
            ...
            if output_step == -1:
                ...
                output_dic['std_list'].append(input_dir['std_profile'] / input_dir['mean_profile'].sum())
            else:
                if all_timesteps:
                    concentration = range(input_dir['last_time_slice'] + 1)
                else:
                    concentration = [output_step]
                # Appending the concentration profiles to 'concentration_list', where the concentrations are now
                # normalized by the total number of particles in the simulation
                for c in concentration:
                    output_dic['concentration_list'].append(input_dir[c] / input_dir[c].sum())
            # Get the depth bins of the concentrations
            output_dic['depth_bins'] = input_dir['bin_edges'] * -1
...
def add_observations(ax, sources=None, wind_range=None, norm_depth=False, alpha=0.5, mean_concentrations=True):
    """
    ...
    #
    if mean_concentrations and wind_range is not None:
        ...
        # Making the actual plot
        ax.errorbar(concentration, -1 * depth, fmt='ok', xerr=std)
        ax.scatter([], [], marker='o', c='k', label='Field data average')
