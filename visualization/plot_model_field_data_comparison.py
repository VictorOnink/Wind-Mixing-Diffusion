                ...
                if kukulka:
                    ...
                    for model in range(len(w_rise_list)):
                        ...
                        ax[scale].plot(concentration, profile_dict['depth_bins'] / correction,
                                       label=label_model_field_comparison(w_rise=w_rise, diffusion_type='Kukulka', boundary=boundary),
        ...
        for boundary in boundary_list:
            if kukulka:
                ...
                for model in range(len(w_10_list)):
                    w_10, w_rise = profile_dict['parameter_kukulka'][model]
                    concentration = profile_dict['concentration_list'][model]
                    ax[scale].plot(concentration, profile_dict['depth_bins'] / correction,
                                   label=label_model_field_comparison(w_10=w_10, diffusion_type='Kukulka', boundary=boundary),
                                   linestyle=utils_v.determine_linestyle(boundary, boundary_list, kpp, kukulka,
                                                                         'Kukulka'),
