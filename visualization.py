...
def markov_1_RMSE_comparison(y_label=r'$u_{10}$ (m s$^{-1}$)', x_label=r'RMSE', fig_size=(16, 8),
                             ax_label_size=16, legend_size=12):
    w_10 = [0.85, 2.4, 4.35, 6.65, 9.3]
    w_r = [-0.03, -0.003]
    ...
    for index_w10, wind in enumerate(w_10):
        for rise in w_r:
            ...
            plot_tuple = RMSE, index_w10 + 1, marker_type[rise], utils.return_color(0)
    ...
    for point in point_list_KPP:
        RMSE, index_w10, marker, color = point
        ax.plot(RMSE, index_w10, color=color, marker=marker, alpha=0.7, markersize=10, mfc=None)
    for point in point_list_Kukulka:
        RMSE, index_w10, marker, color = point
        ax2.plot(RMSE, index_w10, color=color, marker=marker, alpha=0.7, markersize=10, mfc=None)
    ...
    markov1 = [plt.plot([], [], c=utils.return_color(ind+1), markersize=10, marker='o', label=r'M1 - $\alpha = $'+'{}'.format(a), linestyle='')[0] for ind, a in
              enumerate(alpha)]
